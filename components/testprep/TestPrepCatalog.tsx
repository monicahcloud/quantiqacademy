...
import { CourseLevel, Prisma } from "@/lib/generated/prisma/client";
import {
  Search,
  SlidersHorizontal,
  Star,
...
export default async function TestPrepCatalog({
  searchParams,
}: {
  ...
  const where: Prisma.CourseWhereInput = {
    ...
    ...(q && {
      OR: [
        ...
        { exam: { contains: q, mode: "insensitive" } },
  ...
  const exams = [
    ...new Set(
      allCourses.map((course) => course.exam.toUpperCase()).filter(Boolean),
    ),
  ];

  const subjects = [
    ...new Set(allCourses.map((course) => course.subject).filter(Boolean)),
  ];
